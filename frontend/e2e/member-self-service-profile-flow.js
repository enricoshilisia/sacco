...
// email, physical address) via PATCH /api/members/me/ - the third piece of
// "make the member dashboard have their menus" (Loans, Savings, and now
// Profile). Also confirms the Dashboard no longer shows SACCO-wide stats or
...
async function run() {
  ...
  await page.waitForSelector('text=Member details', { timeout: 10000 });
  await page.waitForSelector('text=Amina Njoroge', { timeout: 10000 });

  // --- Contact details are editable; identity fields are not ---
  assert(await page.locator('input').count() === 3, 'exactly 3 editable inputs (phone, email, address)');
