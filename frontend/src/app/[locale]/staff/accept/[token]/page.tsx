...
export default function StaffAcceptPage() {
  ...
  useEffect(() => {
    apiFetch<InviteDetail>(`/api/tenant/staff/invites/${params.token}/accept/`)
      .then((data) => {
        setInvite(data);
        setState("ready");
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const result = await apiFetch<AcceptResult>(`/api/tenant/staff/invites/${params.token}/accept/`, {
        method: "POST",
          ...
          {state === "ready" && invite && (
            <form onSubmit={handleSubmit}>
              ...
              <p className="mt-1 mb-6 text-sm leading-6 text-primary-600">
                {t("welcomeBody", { sacco: invite.sacco_name, role: invite.role_name })}
              </p>
